...
export const transcripts = pgTable('transcripts', {
  id: serial('id').primaryKey(),
  episodeId: integer('episode_id').notNull().references(() => episodes.id).unique(),
...
export const transcriptsRelations = relations(transcripts, ({ one, many }) => ({
  episode: one(episodes, { fields: [transcripts.episodeId], references: [episodes.id] }),
...
export const transcriptChunksRelations = relations(transcriptChunks, ({ one }) => ({
  transcript: one(transcripts, { fields: [transcriptChunks.transcriptId], references: [transcripts.id] }),
  episode: one(episodes, { fields: [transcriptChunks.episodeId], references: [episodes.id] }),
}))
...
export const episodeTopics = pgTable('episode_topics', {
  ...
  primaryKey({ columns: [table.episodeId, table.topicId] }),
])

export const episodeTopicsRelations = relations(episodeTopics, ({ one }) => ({
  episode: one(episodes, { fields: [episodeTopics.episodeId], references: [episodes.id] }),
  topic: one(topics, { fields: [episodeTopics.topicId], references: [topics.id] }),
}))
...
export const episodeEntities = pgTable('episode_entities', {
  ...
  primaryKey({ columns: [table.episodeId, table.entityId] }),
])

export const episodeEntitiesRelations = relations(episodeEntities, ({ one }) => ({
  episode: one(episodes, { fields: [episodeEntities.episodeId], references: [episodes.id] }),
  entity: one(entities, { fields: [episodeEntities.entityId], references: [entities.id] }),
}))

// ── Episode Summaries ─────────────────────────────────────

export const episodeSummaries = pgTable('episode_summaries', {
  id: serial('id').primaryKey(),
  episodeId: integer('episode_id').notNull().references(() => episodes.id).unique(),
