...
export default class PreprocessAudioJob extends Job<PreprocessAudioPayload> {
  ...
  async execute(): Promise<void> {
    ...
    console.log(
      `[PreprocessAudioJob] Episode ${episodeId}: uploaded to storage (${(fileBuffer.length / 1024 / 1024).toFixed(1)}MB)`,
    );
  ...
  async failed(error: Error): Promise<void> {
    ...
    console.error(
      `[PreprocessAudioJob] Episode ${this.payload.episodeId} failed:`,
      error.message,
    );
  }
}
