...
export default class ResolveEntitiesJob extends Job<ResolveEntitiesPayload> {
  ...
  async execute(): Promise<void> {
    ...
    if (!rawEntities || rawEntities.length === 0) {
      console.log(`[ResolveEntitiesJob] Episode ${episodeId}: no entities to resolve`)
    } else {
      console.log(`[ResolveEntitiesJob] Episode ${episodeId}: resolving ${rawEntities.length} entities`)
      await resolveEntities(db, episodeId, rawEntities)
      console.log(`[ResolveEntitiesJob] Episode ${episodeId}: entity resolution complete`)
    }
  ...
  async failed(error: Error): Promise<void> {
    ...
    console.error(`[ResolveEntitiesJob] Episode ${this.payload.episodeId} failed:`, error.message)
  }
}
