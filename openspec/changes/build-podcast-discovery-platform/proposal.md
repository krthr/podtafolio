...
- Add AI-powered content extraction: ad stripping, topic/entity extraction, episode summarization using Gemini Flash
- Add entity resolution system using pgvector similarity to deduplicate entities across episodes and language variations
- Add semantic search with AI-synthesized answers (Gemini Pro) and a semantic answer cache with topic-aware invalidation
...
- `content-analysis`: LLM-based analysis pass (Gemini Flash) that strips ads, extracts topics, extracts entities, and generates episode summaries from transcripts
- `entity-resolution`: Deduplicating entities using pgvector cosine similarity and Gemini Flash, maintaining canonical entities with aliases
- `semantic-search`: Embedding transcript chunks in pgvector, hybrid vector + full-text search, and AI-synthesized answers (Gemini Pro) with semantic caching and topic-aware invalidation
...
- **Dependencies**: nuxt, podcastindex, @boringnode/queue, @google/generative-ai (Gemini), groq-sdk, fluent-ffmpeg (or child_process ffmpeg), pg + pgvector extension, an embedding model client
