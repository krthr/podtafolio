<template>
  <div v-if="data">
    ...
    <p v-if="data.entity.description" class="description">
    ...
    <section v-if="data.episodeMentions.length > 0" class="section">
      ...
      <div class="episodes-list">
        <article
          v-for="ep in data.episodeMentions"
          :key="ep.episodeId"
          class="episode-card"
        >
          ...
          <div v-if="ep.contextSnippets" class="snippets">
            <p
              v-for="(snippet, i) in (ep.contextSnippets as string[]).slice(
                0,
                2,
              )"
              :key="i"
...
if (!data.value) {
  throw createError({
    statusCode: 404,
    statusMessage: "Entidad no encontrada",
  });
}
</script>
