  ...
  <div v-if="data">
    <div class="podcast-header">
      ...
      <div>
        ...
        <p v-if="data.podcast.description" class="description">{{ data.podcast.description }}</p>
...
if (!data.value) {
  throw createError({ statusCode: 404, statusMessage: 'Podcast no encontrado' })
}
</script>

<style scoped>
.podcast-header { display: flex; gap: 1.5rem; align-items: flex-start; margin-bottom: 2rem; }
.podcast-artwork { width: 120px; height: 120px; border-radius: 0.75rem; object-fit: cover; flex-shrink: 0; }
.page-title { font-size: 1.5rem; margin-bottom: 0.5rem; }
.description { color: #4b5563; font-size: 0.875rem; margin-bottom: 0.5rem; line-height: 1.5; }
...
.episode-duration { color: #6b7280; }
.status { font-size: 0.6875rem; padding: 0.125rem 0.375rem; border-radius: 0.25rem; text-transform: uppercase; }
.status-done { background: #d1fae5; color: #065f46; }
.status-pending { background: #fef3c7; color: #92400e; }
.status-processing { background: #dbeafe; color: #1e40af; }
.status-failed { background: #fee2e2; color: #991b1b; }
.episode-title { font-weight: 600; color: #1a1a1a; display: block; margin-bottom: 0.25rem; }
.episode-title:hover { color: #2563eb; text-decoration: none; }
.episode-summary { font-size: 0.875rem; color: #4b5563; line-height: 1.5; }
</style>
